...
Route::middleware('auth:sanctum')->group(function () {
    Route::middleware('role:admin')->group(function () {
        Route::prefix('admin')->group(function () {
            Route::get('/statistics', [AdminStatisticController::class, 'summary']);
            Route::get('/tutor/verify', [TutorVerifyController::class, 'index']);
            Route::patch('/tutor/{userId}/verify/approve', [TutorVerifyController::class, 'approve'])
                ->whereNumber('userId');
            Route::patch('/tutor/{userId}/verify/reject', [TutorVerifyController::class, 'reject'])
                ->whereNumber('userId');

            // Payout
            Route::get('/payout', [AdminPayoutController::class, 'index']);
            Route::patch('/payout/{id}/approve', [AdminPayoutController::class, 'approve'])->whereNumber('id');
            Route::patch('/payout/{id}/reject', [AdminPayoutController::class, 'reject'])->whereNumber('id');
        });
    });
});
